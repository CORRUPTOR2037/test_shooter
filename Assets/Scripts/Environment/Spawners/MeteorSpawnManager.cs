...
{
    private Dictionary<int, BaseSpawnPlace> spawnPlaces;
    ...
    public IEnumerable Meteors => enumerator;
    ...
    private IEnumerator SpawnMeteorsCoroutine(LevelWave wave)
    {
        ...
        while (Count != 0)
        {
            ...
            Vector2 nextPosition = spawnPlaces[nextSpawnerIndex].NextPosition();
    ...
    private BaseMeteor GetFreeMeteor(MeteorModel model)
    {
        if (!meteorPools.ContainsKey(model)) {
            meteorPools[model] = new ObjectPool<BaseMeteor>(() => MeteorFactory.Create(model), container: LevelController.Current.MeteorsContainer);
        }
